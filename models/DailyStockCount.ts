...
const DailyStockCountItemSchema =
	new Schema<DailyStockCountItem>(
		{
			productId: {
				type: String,
				required: true,
...
const DailyStockCountSchema =
	new Schema<DailyStockCountDoc>(
		{
			...
			items: {
				type: [DailyStockCountItemSchema],
				default: [],
			},
			countedByUserId: {
				type: String,
...
DailyStockCountSchema.index(
	{ date: 1, sessionId: 1 },
	{ unique: true },
);
DailyStockCountSchema.index({ date: 1, status: 1 });

export const DailyStockCount =
	getModel<DailyStockCountDoc>(
		"DailyStockCount",
