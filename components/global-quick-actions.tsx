...
export function GlobalQuickActions() {
	...
	const date = useMemo(() => getTodayJHB(), []);
	...
	const [adjustmentItems, setAdjustmentItems] =
		useState<AdjustmentItemForm[]>([
			blankAdjustmentItem(),
		]);
	...
	const [
		paymentCashReceivedCents,
		setPaymentCashReceivedCents,
	] = useState(0);
	...
	const [
		expenseAmountCents,
		setExpenseAmountCents,
	] = useState(0);
	const [expenseReason, setExpenseReason] =
		useState("");
	const [expenseCategory, setExpenseCategory] =
		useState<(typeof EXPENSE_CATEGORIES)[number]>(
			"OTHER",
		);
	const [expensePayee, setExpensePayee] =
		useState("");
	const [expenseReference, setExpenseReference] =
		useState("");
	...
	const [customerMode, setCustomerMode] =
		useState<"ACCOUNT" | "DEBT_ONLY">("ACCOUNT");
	const [
		customerTemporaryTab,
		setCustomerTemporaryTab,
	] = useState(false);
	const [
		customerOpeningBalanceCents,
		setCustomerOpeningBalanceCents,
	] = useState(0);
	const [
		customerCreditLimitCents,
		setCustomerCreditLimitCents,
	] = useState(0);
	const [customerDueDays, setCustomerDueDays] =
		useState("");
	...
	const [
		...
		setSupplierPriceSupplierId,
	] = useState(NO_VALUE);
	const [
		supplierPriceProductId,
		setSupplierPriceProductId,
	] = useState("");
	const [
		supplierPriceCostCents,
		setSupplierPriceCostCents,
	] = useState(0);
	const [
		supplierPriceEffectiveFrom,
		setSupplierPriceEffectiveFrom,
	] = useState(date);
	const [
		supplierPriceMoqUnits,
		setSupplierPriceMoqUnits,
	] = useState("");
	const [
		supplierPriceLeadTimeDays,
		setSupplierPriceLeadTimeDays,
	] = useState("");
	const [
		supplierPriceNote,
		setSupplierPriceNote,
	] = useState("");

	const [productName, setProductName] =
		useState("");
	const [productCategory, setProductCategory] =
		useState<ProductCategory>("Beer");
	const [productBarcode, setProductBarcode] =
		useState("");
	const [productPackSize, setProductPackSize] =
		useState("24");
	const [
		productReorderLevel,
	...
	useEffect(() => {
		if (activeAction === "restock") {
			setPurchaseSupplierId(
				restockSeed.initialSupplierId || NO_VALUE,
			);
			setPurchaseItems(
		...
		if (activeAction === "opening-stock") {
			setAdjustmentItems([
				blankAdjustmentItem("OPENING_STOCK"),
			]);
		}
	...
	const getPurchaseUnits = useCallback(
	...
	const savePurchase = async () => {
		const items = purchaseItems
			.map((item) => ({
				...item,
				units: getPurchaseUnits(item),
			}))
			.filter(
				(item) =>
					item.productId && item.units > 0,
			)
			.map((item) => ({
				productId: item.productId,
	...
	const saveAdjustment = async () => {
		const openingStockMode =
			activeAction === "opening-stock";
		const items = adjustmentItems
			.filter(
				(item) =>
					item.productId &&
					toInt(item.unitsDelta) !== 0 &&
					(openingStockMode || item.reason),
			)
			.map((item) => ({
				productId: item.productId,
				unitsDelta: toInt(item.unitsDelta),
				reason: openingStockMode
					? "OPENING_STOCK"
					: item.reason,
		...
		await submitJson(
			...
			openingStockMode
				? "Existing stock saved"
				: "Adjustment saved",
			openingStockMode
				? "Failed to save existing stock"
	...
	const saveCustomer = async () => {
		await submitJson(
			postJson("/api/customers", {
				name: customerName,
				phone:
					optionalText(customerPhone) ?? null,
				...
				creditLimitCents:
					customerCreditLimitCents,
				dueDays: optionalText(customerDueDays)
					? toInt(customerDueDays)
					: undefined,
			}),
			"Customer saved",
	...
	const saveSupplierPrice = async () => {
		if (
			supplierPriceSupplierId === NO_VALUE ||
			!supplierPriceProductId
		) {
		...
		await submitJson(
			postJson("/api/supplier-prices", {
				...
				moqUnits: optionalText(
					supplierPriceMoqUnits,
				)
					? toInt(supplierPriceMoqUnits)
					: undefined,
				leadTimeDays: optionalText(
					supplierPriceLeadTimeDays,
				)
	...
	const saveProduct = async () => {
		await submitJson(
			postJson("/api/products", {
				...
				packSize: Math.max(
					1,
					toInt(productPackSize),
				),
				reorderLevelUnits: Math.max(
					0,
					toInt(productReorderLevel),
				),
			}),
			"Product saved",
	...
	const pagePrimaryActions = useMemo<
		QuickAction[]
	>(() => {
	...
	const renderQuickActionButton = (
		...
		const label = getQuickActionLabel(
			action,
											...
											(action) => {
												...
												return (
													...
													>
														...
														<Star
															className={`h-4 w-4 ${pinned ? "fill-current text-amber-500" : "text-muted-foreground"}`}
														/>
													</Button>
												);
											},
	...
	function renderDialogContent() {
		...
		if (
			...
			return renderAdjustmentDialog(
				activeAction === "opening-stock",
			);
		}
		if (activeAction === "account-payment")
			return renderPaymentDialog();
		if (activeAction === "expense")
			return renderExpenseDialog();
		if (activeAction === "customer")
			return renderCustomerDialog();
		if (activeAction === "supplier")
			return renderSupplierDialog();
		if (activeAction === "supplier-price")
			return renderSupplierPriceDialog();
		if (activeAction === "product")
			return renderProductDialog();
		return null;
	}
	...
	function renderAdjustmentDialog(
		...
		return (
			<>
				<DialogHeader>
					...
					<DialogDescription>
						Record a customer payment against an
				...
				<div className="space-y-4">
					...
					{paymentMethod === "CASH" && (
						<div className="rounded-lg border p-3 space-y-2">
							<MoneyInput
								label="Cash received"
								value={paymentCashReceivedCents}
								onChange={
									setPaymentCashReceivedCents
								}
							/>
							<p className="text-sm text-muted-foreground">
								Change: {formatZAR(changeCents)}
							</p>
						</div>
					)}
					<div className="grid gap-3 md:grid-cols-2">
		...
		return (
			<>
				<DialogHeader>
					...
					<DialogDescription>
						Record non-stock cash leaving the
						business.
					</DialogDescription>
				</DialogHeader>
				<div className="space-y-4">
					<div className="grid gap-3 md:grid-cols-2">
						<MoneyInput
							label="Amount"
							value={expenseAmountCents}
							onChange={setExpenseAmountCents}
						/>
						<div className="space-y-2">
		...
		return (
			<>
				<DialogHeader>
					...
					<DialogDescription>
						Create a customer/account contact.
					</DialogDescription>
				</DialogHeader>
				<div className="space-y-4">
					<div className="grid gap-3 md:grid-cols-2">
					...
					<div className="grid gap-3 md:grid-cols-2">
						<MoneyInput
							...
							value={customerOpeningBalanceCents}
							onChange={
								setCustomerOpeningBalanceCents
							}
						/>
						<MoneyInput
							label="Credit limit"
							value={customerCreditLimitCents}
							onChange={
								setCustomerCreditLimitCents
							}
						/>
		...
		return (
			<>
				<DialogHeader>
					...
					<DialogDescription>
						Save the current cost for a product
						from a supplier.
					</DialogDescription>
				</DialogHeader>
				<div className="space-y-4">
					<div className="grid gap-3 md:grid-cols-2">
						...
						<ProductSelect
							products={products}
							value={supplierPriceProductId}
							onChange={setSupplierPriceProductId}
						/>
					</div>
					<div className="grid gap-3 md:grid-cols-3">
						<MoneyInput
							label="Unit cost"
							value={supplierPriceCostCents}
							onChange={setSupplierPriceCostCents}
						/>
						<div className="space-y-2">
							<Label>Effective from</Label>
							<Input
								type="date"
								value={supplierPriceEffectiveFrom}
								onChange={(event) =>
									setSupplierPriceEffectiveFrom(
										event.target.value,
									)
					...
					<LabeledInput
						...
						value={supplierPriceLeadTimeDays}
						onChange={(value) =>
							setSupplierPriceLeadTimeDays(
								cleanIntInput(value),
							)
	...
	function updateAdjustmentItem(
		index: number,
		patch: Partial<AdjustmentItemForm>,
	) {
		setAdjustmentItems((prev) =>
			prev.map((item, itemIndex) =>
				itemIndex === index
